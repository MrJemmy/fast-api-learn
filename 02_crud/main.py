...
app = FastAPI(
    title="Book CRUD API",
    description="A simple, best-practice implementation of CRUD operations for a Book resource.",
)
...
class BookBase(BaseModel):
    ...
    publisher: str = Field(..., example="Pan Books")
    published_date: date = Field(
        ..., example=date(1979, 10, 12), description="The date the book was first published."
    )
    # gt=0 ensures the count is greater than zero
    page_count: int = Field(..., gt=0, example=224)
    language: str = Field(..., example="English")

    # Optional Fields
    isbn: Optional[str] = Field(None, example="978-0345391803")
    is_published: Optional[bool] = Field(None, example=True)

# Define BookCreate explicitly for clarity (inherits all fields from BookBase)
...
@app.get("/")
async def read_root():
    return {"message": "Welcome to the Book CRUD API. Go to /docs for interactive documentation."}

# ==============================================================================
# 🚀 CRUD Path Operations (Best Implementation)
# ==============================================================================
...
async def create_book(book_data: BookBase):
    ...
    # **REQUEST DATA ACCESS:** FastAPI provides the validated Pydantic object `book_data`
...
async def read_book(book_id: int):
    ...
    # **REQUEST DATA ACCESS:** Path parameter `book_id` is automatically parsed as int
...
# UPDATE (PUT)
# Uses Pydantic (BookBase) for request data and HTTPException for error status_code=404


@app.put("/books/{book_id}", response_model=Book)
async def update_book(book_id: int, book_data: BookBase):  # Corrected to use BookBase
    """Updates an existing book by ID."""
    # **REQUEST DATA ACCESS:** Access path parameter `book_id` and body `book_data`
    if book_id not in BOOKS_DB:
...
# DELETE (DELETE)
# Uses status_code=204 for success (no content returned) and HTTPException for error status_code=404
